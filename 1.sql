...
-- 1
CREATE OR REPLACE FUNCTION most_user_created_in_year()
RETURNS integer 
    AS $$
    BEGIN 
...
CREATE OR REPLACE FUNCTION birth_month(input_month INT)
RETURNS TABLE (user_id INT, user_first_name VARCHAR, user_last_name VARCHAR, user_dob DATE)
    AS $$
    BEGIN
        RETURN QUERY
        SELECT users.user_id, users.user_first_name, users.user_last_name, users.user_dob
        FROM users 
        WHERE EXTRACT(MONTH FROM users.user_dob) = input_month;
        
    END;
$$  LANGUAGE plpgsql;        

-- 3
CREATE OR REPLACE FUNCTION get_joined_year(input_year numeric)
RETURNS TABLE (user_first_name VARCHAR)
    ...
    BEGIN
        ...
        FROM users 
        WHERE EXTRACT(YEAR FROM users.created_ts) = input_year;
        
    END;
$$  LANGUAGE plpgsql; 

-- 4
CREATE OR REPLACE FUNCTION get_gender_count(input_gender VARCHAR)
RETURNS TABLE (user_first_name VARCHAR) 
AS $$
BEGIN
    RETURN QUERY
    SELECT users.user_first_name
    FROM users 
    WHERE users.user_gender = input_gender;
